...
Table::Table(const Table &a){
    ...
    for(int col=0; col<szCol; col++){
        ...
        for(int row=0; row<szRow; row++){
            int value = a.getData(row, col);
            columns[col]->setData(row, value);
        }
    }
}
...
PlusTab PlusTab::operator+(const PlusTab &a){
    PlusTab result(getSzRow(), getSzCol());
    
    for(int col=0; col<getSzCol(); col++){
        for(int row=0; row<getSzRow(); row++){
            int sum = getData(row, col) + a.getData(row, col);
