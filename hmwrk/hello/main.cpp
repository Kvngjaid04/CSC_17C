...
int main(int argc, char** argv) {
    ...
    std::mt19937 mt(rd());
    std::uniform_int_distribution<int> dist(1, 1000000); // Using a larger range for better distribution
...
void savCSV(const vector<int>& sizes, const vector<float>& hpTims, 
    ...
    outFile.close();
    cout << "Results saved to Timing_analysis.csv" << endl;
}
...
void genRand(int arr[], int n) {
    ...
    for (int i = 0; i < n; i++) {
        arr[i] = dist(mt);  // Generate truly random numbers in range [1, n*10]
    }
}
